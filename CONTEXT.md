...
**操作（Action）**:
用户从按钮触发的组合动作，可混含补丁操作与 Steam 启动/退出（应用补丁并启动 / 正常启动 / 退出并卸载 / 卸载并重启 / 重启）。
_Avoid_: 步骤、任务

**重启（Restart）**:
「重启 Steam」：关闭 Steam 进程组后重新启动 `steam.exe` 的纯 Steam 组合动作，不含补丁部署/卸载；已应用且 Steam 运行中时可用（操作区按钮 / 托盘菜单入口），不经确认框（按钮语义即关闭并重启）。
_Avoid_: 重启并卸载（那是「卸载补丁并重启 Steam」）
**本地版本（Local Version）**:
...
**更新流程（Update Flow）**:
在线更新「检查更新」结果的唯一事实源与派生模块（`update_flow`）：持有一次检查结果状态（未检查/检查中/已检查），`derived` 从「本地版本 vs 线上版本」派生线上版本行文案、通知文案与下载可用性；下载成功后本地版本更新即自然重派生。
_Avoid_: 更新状态、检查状态
...
**忙碌门禁（Busy Gate）**:
交互类后台操作（动作 / 更新动作）互斥的单一入口：`busy.rs` 的 `BusyGate` 持有唯一的忙碌种类值，同时刻仅一个操作放行，阶段文案与按钮禁用由它统一产出。
_Avoid_: 忙碌标志、互斥锁

**托盘（Tray）**:
Windows 系统托盘图标，窗口隐藏到托盘后的恢复入口、退出出口与快捷操作入口（重启 Steam，仅 Steam 运行中可点）。
_Avoid_: 系统栏、状态栏图标
...
**关闭完成（Close Settled）**:
「关闭 Steam」的成功态：Steam 进程组内所有进程从进程表消失；先 `steam.exe -shutdown` 优雅退出（轮询预算 10s），超时回退硬杀（轮询预算 5s），仍超时视为关闭失败。
_Avoid_: 关闭成功（易与 spawn 成功混淆）

**设置对话框（Settings Dialog）**:
顶栏「设置」按钮打开的模态对话框，按页签组织非主流程设置：当前含「配置编辑器」与「OnlineFix 启动预设」两页，页签互不共享状态，会话内记忆上次页签。
_Avoid_: 设置窗口、选项框
...
**自动预热（Auto-precache）**:
体检落定「上游已适配（未缓存）」且无预热进行中时自动触发的下载动作，用户零操作；失败静默（徽章保持 Online、手动入口保留），成功复用「缓存已就绪」提示。
_Avoid_: 自动下载、后台预热

**体检流程（Compat Flow）**:
兼容性体检生命周期的编排状态机（`compat_flow` 模块）：按「每体检代数一次网络刷新」串联快速体检、后台网络刷新、自动/手动预热与复检；路径变更推进代数、迟到的旧结果被丢弃，刷新与预热并行在途互不抹除。App 只喂事件、执行返回的效果并渲染展示态。
_Avoid_: 体检编排、流程控制
...
- 「重启」是纯 Steam 组合动作（关闭+重新启动），不含补丁操作；与「卸载补丁并重启 Steam」（含卸载）区分。
...
- 「关闭 Steam」优先优雅退出（`steam.exe -shutdown`，预算 10s），超时回退硬杀进程组（预算 5s）；不做无条件的 TerminateProcess（强杀触发看门狗自重启是「重启失败」的根因，见 ADR-0004 修订）。
...
- 「自动预热」只在体检落定「上游已适配（未缓存）」且无预热进行中时触发；失败静默不弹错，成功复用「缓存已就绪」提示。
- 「忙碌门禁」只互斥交互类后台操作（动作 + 更新动作）；compat 探针/刷新/预热是只读后台操作，不进忙碌门禁，其互斥由「体检流程」自有的在途去重承担。
- 「检查更新」结果记录最后一次检查时的线上版本，下载成功后不清除；下载按钮可用性由「本地版本 == 记录版本」派生，而非「下载过就隐藏」。
