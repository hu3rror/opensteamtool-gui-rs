...
参考原版（宽 560 × 高 470，不可缩放，居中）：

1. **卡片 1 — Steam 安装路径**：输入框（可编辑，变更即刷新状态）+ 「浏览...」按钮（目录选择器）。
2. **卡片 2 — 本地部署状态**：状态文本 + 两个主操作按钮（随状态切换）：
   - 未部署：左「应用补丁并启动 Steam」/ 右「正常启动 Steam」
   - 已部署：左「退出 Steam 并卸载补丁」/ 右「卸载补丁并重启 Steam」
3. **卡片 3 — 在线版本更新**：本地版本显示 + 线上版本状态 + 「检查更新」按钮 + 「下载并解压新版本」按钮（有可更新版本时出现）。
