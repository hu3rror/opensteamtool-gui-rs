...
窗口在检测到 Steam 进程运行时自动隐藏到 Windows 托盘，Steam 退出后自动恢复并聚焦。决定：任何来源的 `steam.exe` 运行/退出都触发（不只是本工具按钮启动的）；托盘左键单击/双击均切换窗口显隐；托盘右键菜单含「显示」「最小化时自动隐藏到托盘」（勾选项，默认开）「退出」；窗口关闭（X）直接退出程序（托盘不做常驻，仅服务自动隐身、最小化隐身与手动显隐切换）。
